...
This document describes version 1.00 of Class::Multimethods,
...
The Class:Multimethod module exports a single subroutine that can be
...
The Class::Multimethods module exports a single subroutine,
...
in an exception being thrown, with the message: C<"No viable candidate
for call to multimethod LargeNumeric::divide at I<linenumber>">.

Alternatively, we could provide a "catch-all" variant:
...
"numeric" has the usual Perl meaning: a scalar value C<$s> is numeric
if C<$s+0 eq $s>).
...
For example:
	...
	# Special cases...
	    ...
	    multimethod handle => (MoveableWindow, MoveEvent, NormalMode)
		=> sub { ... }
